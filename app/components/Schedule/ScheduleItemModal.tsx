...
export function ScheduleItemModal({
  ...
  const [courtId, setCourtId] = useState(
    String(item?.court_id ?? courts.find((court) => Boolean(court.is_active))?.id ?? ""),
  );
  return (
    <div
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          save({
            ...
            is_tournament_wide: wide,
            court_id: wide ? null : Number(courtId),
          });
        }}
      >
        <div className="modal-header">
          <div>
            <p>PROGRAMMAONDERDEEL</p>
            <h2 id="item-title">{item ? "Onderdeel wijzigen" : "Onderdeel toevoegen"}</h2>
          </div>
        ...
        <div className="modal-grid">
          <label className="modal-wide">
            ...
            <input
              ...
              placeholder="Bijvoorbeeld prijsuitreiking"
            />
          </label>
          <label>
            Type
            <select value={type} onChange={(event) => setType(event.target.value as ScheduleItem["item_type"])}>
              <option value="custom">Overig</option>
              <option value="break">Pauze</option>
              <option value="ceremony">Ceremonie</option>
              <option value="maintenance">Baanonderhoud</option>
            </select>
          </label>
          <label>
            Starttijd
          ...
          <label>
            Locatie
            <select
              value={wide ? "wide" : courtId}
              onChange={(event) => {
                setWide(event.target.value === "wide");
                if (event.target.value !== "wide") setCourtId(event.target.value);
              }}
            >
              <option value="wide">Toernooibreed</option>
              {courts
