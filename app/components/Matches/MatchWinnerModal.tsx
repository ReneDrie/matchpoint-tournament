...
export function MatchWinnerModal({
  ...
  return (
    ...
    >
      <div
        className="modal-card match-confirm-modal"
        role="dialog"
        aria-modal="true"
        aria-labelledby="winner-confirm-title"
      >
        <div className="modal-header">
          <div>
            <p>{correcting ? "UITSLAG CORRIGEREN" : "WINNAAR BEVESTIGEN"}</p>
            <h2 id="winner-confirm-title">{winnerName} wint de wedstrijd?</h2>
          </div>
          <button type="button" className="modal-close" onClick={close} aria-label="Sluiten">
            ×
          </button>
        </div>
        <div className="winner-confirm-player">
          <span>✓</span>
          <div>
        ...
        {correcting && (
          <p className="winner-warning">
            De bestaande uitslag wordt gecorrigeerd. Eventuele uitslagen in vervolgrondes die hiervan afhangen worden
            veilig gewist.
          </p>
        )}
        <div className="modal-actions">
          ...
          <button type="button" className="primary" onClick={confirm} disabled={busy}>
            {busy ? "Opslaan…" : correcting ? "Uitslag corrigeren" : "Winnaar bevestigen"}
          </button>
        </div>
      </div>
    </div>
