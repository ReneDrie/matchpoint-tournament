...
export function useSponsors({
  ...
  async function playerSaved() {
    await Promise.all([reloadPlayers(), reloadSponsors()]);
  }
  return {
    activeTab,
    setActiveTab,
...
export function useSponsorForm({
  ...
  const [tierId, setTierId] = useState(
    sponsor?.tier_id ? String(sponsor.tier_id) : tiers[0]?.id ? String(tiers[0].id) : "",
  );
  ...
  const [playerLimitOverride, setPlayerLimitOverride] = useState(
    sponsor?.player_limit_override === null || sponsor?.player_limit_override === undefined
      ? ""
      : String(sponsor.player_limit_override),
  );
  ...
  async function submit(event: FormEvent) {
    ...
    const response = await fetch(
      sponsor ? `${API_URL}/api/admin/sponsors/${sponsor.id}` : `${API_URL}/api/admin/sponsors`,
      {
        ...
        body: JSON.stringify({
          ...
          website_url: website,
          player_limit_override: playerLimitOverride === "" ? null : Number(playerLimitOverride),
          is_active: isActive,
          show_on_public_pages: showPublic,
        }),
      },
    );
    ...
    if (logoFile) {
      ...
      const logoResponse = await fetch(`${API_URL}/api/admin/sponsors/${result.sponsor_id}/logo`, {
        method: "POST",
        credentials: "include",
      ...
      if (!logoResponse.ok) {
        setError(logoResult.error ?? "Sponsorlogo kon niet worden opgeslagen.");
        setBusy(false);
        return;
      }
    }
...
export function useSponsorPackageForm({
  user,
  sponsorPackage,
  saved,
  close,
  ...
  user: StaffUser;
  sponsorPackage?: SponsorTier;
  saved: () => Promise<void>;
  close: () => void;
}) {
  const [name, setName] = useState(sponsorPackage?.name ?? "");
  const [cost, setCost] = useState(
    sponsorPackage ? (sponsorPackage.cost_cents / 100).toFixed(2).replace(".", ",") : "0,00",
  );
  ...
  async function submit(event: FormEvent) {
    ...
    setError("");
    const response = await fetch(
      sponsorPackage ? `${API_URL}/api/admin/sponsor-tiers/${sponsorPackage.id}` : `${API_URL}/api/admin/sponsor-tiers`,
      {
        method: sponsorPackage ? "PATCH" : "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json", "X-CSRF-Token": user.csrf_token },
        body: JSON.stringify({ name, cost_cents: costCents, included_players: includedPlayers }),
      },
    );
    const result = await response.json();
    if (!response.ok) {
      setError(result.error ?? "Sponsorpakket kon niet worden opgeslagen.");
      setBusy(false);
      return;
    }
    await saved();
  ...
  async function remove() {
    ...
    setBusy(true);
    setError("");
    const response = await fetch(`${API_URL}/api/admin/sponsor-tiers/${sponsorPackage.id}`, {
      method: "DELETE",
      credentials: "include",
      headers: { "X-CSRF-Token": user.csrf_token },
    });
    const result = await response.json();
    if (!response.ok) {
      setError(result.error ?? "Sponsorpakket kon niet worden verwijderd.");
      setBusy(false);
      return;
    }
    await saved();
