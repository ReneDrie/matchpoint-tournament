...
export function Overview({ setView, players, tournament }: { setView: Navigate; players: Player[]; tournament: TournamentConfig | null }) {
  ...
  return <>
    <section className="stats"><Stat value={String(confirmed)} label="Betaalde deelnemers" note={`${reservedSponsorSpots} sponsorplek${reservedSponsorSpots === 1 ? "" : "ken"} nog gereserveerd`} badge={`Nog ${publicSpots} publieke plekken`} accent /><Stat value={`${occupancy}%`} label="Bezetting" note={`Inschrijven sluit ${deadline}`} badge={tournament?.registration_available ? "Inschrijving open" : "Inschrijving gesloten"} /><Stat value={String(tournament?.active_courts ?? 2)} label="Actieve banen" note="Beheerbaar via instellingen" badge="Beschikbaar" /><Stat value={String(firstRoundMatches)} label="Wedstrijden ronde 1" note={`Toernooi start om ${start.toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit" })} uur`} badge="Nog te plannen" /></section>
    <section className="overview-grid"><div className="panel upcoming"><div className="panel-title"><div><p>LIVE OPERATIE</p><h2>Eerstvolgende wedstrijden</h2></div><button onClick={() => setView("matches")}>Alle wedstrijden →</button></div><div className="empty-state"><strong>Nog geen wedstrijden gepland</strong><span>Wedstrijden verschijnen hier nadat de handmatige loting is ingevuld.</span></div></div><div className="panel progress"><div className="panel-title"><div><p>TOERNOOI VOORTGANG</p><h2>{roundCount} rondes naar de winnaar</h2></div></div><div className="rounds">{Array.from({ length: roundCount }, (_, index) => <span key={index} className={index === 0 ? "current" : ""} />)}</div><div className="round-labels"><strong>Ronde 1</strong><span>Finale</span></div><div className="progress-big"><strong>0</strong><span>van {capacity - 1}<br />wedstrijden gespeeld</span></div><button className="secondary" onClick={() => setView("matches")}>Open wedstrijdbeheer</button></div></section>
