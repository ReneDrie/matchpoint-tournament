...
export function Sidebar({ view, user, navigate, logout }: { view: View; user: StaffUser; navigate: Navigate; logout: () => void }) {
  return <aside className="sidebar">
    ...
    <nav>{nav.filter(item => user.role === "administrator" || !["draw", "sponsors", "presentation", "settings"].includes(item.id)).map(item => <a key={item.id} href={routeHref(viewRoutes[item.id])} className={view === item.id ? "active" : ""} onClick={event => navigate(item.id, event)}><span>{item.icon}</span>{item.label}</a>)}</nav>
...
export function Topbar({ view, user, navigate, tournament }: { view: View; user: StaffUser; navigate: Navigate; tournament: { name: string; starts_at: string } | null }) {
  const labels: Record<View, string> = { overview: `Goedemorgen, ${user.name.split(" ")[0]}`, players: "Deelnemers", draw: "Handmatige loting", matches: "Wedstrijden", schedule: "Court planning", sponsors: "Sponsors", presentation: "Presentatiemodus", settings: "Toernooi-instellingen", registration: "Inschrijving" };
