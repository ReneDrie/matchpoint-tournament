...
export function Sidebar({
  ...
  return (
    <aside className="sidebar">
      ...
      <nav>
        {nav
          ...
          .map((item) => (
            <a
              key={item.id}
              href={routeHref(viewRoutes[item.id])}
      ...
      <a className="public-link" href={routeHref("/")} onClick={(event) => navigate("registration", event)}>
        <span>↗</span> Inschrijfpagina
      </a>
      <div className="profile">
        ...
        <div>
          ...
          <small>{user.role === "administrator" ? "Administrator" : "Host"}</small>
        </div>
        <button onClick={logout} title="Uitloggen">
          ↪
        </button>
      </div>
    </aside>
...
export function Topbar({
  ...
  user: StaffUser;
  navigate: Navigate;
  tournament: { name: string; starts_at: string } | null;
}) {
  const labels: Record<View, string> = {
    overview: `Goedemorgen, ${user.name.split(" ")[0]}`,
    players: "Deelnemers",
    draw: "Handmatige loting",
    matches: "Wedstrijden",
  ...
  const date = tournament
    ? new Date(tournament.starts_at.replace(" ", "T")).toLocaleDateString("nl-NL", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })
  ...
  return (
    <header className="topbar">
      <div>
        <p>
          {tournament?.name ?? "Matchpoint Tournament"} · {date}
        </p>
        <h1>{labels[view]}</h1>
      </div>
      <div className="top-actions">
        <a
          ...
          href={routeHref("/inschrijven")}
          onClick={(event) => navigate("registration", event)}
        >
          ＋ Nieuwe inschrijving
        </a>
      </div>
