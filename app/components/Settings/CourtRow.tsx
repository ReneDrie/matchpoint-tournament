...
export function CourtRow({ court, user, saved, onError }: { court: Court; user: StaffUser; saved: () => Promise<void>; onError: (message: string) => void }) {
  ...
  return <div className={`court-setting-row ${editor.isActive ? "" : "inactive"} ${editor.busy ? "saving" : editor.justSaved ? "saved" : ""}`} aria-busy={editor.busy}>
    ...
    <div className="court-actions"><span className="sr-only" aria-live="polite">{editor.busy ? "Opslaan" : editor.justSaved ? "Automatisch opgeslagen" : ""}</span><button className="secondary" type="button" onClick={editor.remove} disabled={editor.busy}>Verwijderen</button></div>
  </div>;
}
