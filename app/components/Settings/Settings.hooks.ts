...
type SettingsForm = Omit<
  ...
  | "venue"
  | "confirmed_players"
  | "sponsor_reserved_spots"
  | "public_spots_available"
...
function formFromTournament(tournament: TournamentSettings): SettingsForm {
  return {
    ...tournament,
    starts_at: toInputDate(tournament.starts_at),
    registration_deadline_at: toInputDate(tournament.registration_deadline_at),
  };
}
    ...
    try {
      const response = await fetch(`${API_URL}/api/admin/tournaments/${tournamentId}`, {
        ...
        body: JSON.stringify({
          ...
          registration_deadline_at: toApiDate(form.registration_deadline_at),
        }),
      });
...
export function useCourtEditor({
  ...
  const updateSurface = (value: string) => {
    setSurface(value);
    setJustSaved(false);
  };
  ...
  function save(overrides: { name?: string; surface?: string; isActive?: boolean } = {}) {
    ...
    if (nextName === court.name && nextSurface === (court.surface ?? "") && nextIsActive === Boolean(court.is_active))
