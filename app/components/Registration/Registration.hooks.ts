...
export function useRegistrationForm() {
  const invitationToken = typeof window === "undefined" ? "" : new URLSearchParams(window.location.search).get("uitnodiging") ?? "";
  ...
  async function submit(event: FormEvent) {
    ...
    setError("");
    try {
      const response = await fetch(`${API_URL}/api/registrations`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, invitation_token: invitationToken || undefined }) });
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/waitlist/invitation?token=${encodeURIComponent(invitationToken)}`).then(async response => {
      const result = await response.json();
    ...
    }).catch(cause => setError(cause instanceof Error ? cause.message : "Deze uitnodiging is niet geldig.")).finally(() => setInvitationLoading(false));
  }, [invitationToken]);

  return { step, setStep, busy, error, form, update, personalComplete, readyToPay, submit, invitationToken, invitationLoading, invitationValid };
}
